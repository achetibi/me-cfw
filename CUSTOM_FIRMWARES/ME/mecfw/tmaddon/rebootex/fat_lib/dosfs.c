...
*/
static uint8_t *DFS_CanonicalToDir(uint8_t *dest, uint8_t *src)
{
...
char *wcharcpy(char *str , const char *origin , int size )
{
	...
	while( size-- )
	{
		*str++ = *origin;//tolower( *origin );
		origin += 2;
	}
	return ret;
}

void cache_set( int flag )
{
	cache_flag = flag;
	if( flag)
		memset( LFN_buff , 0 , 128 );
}

int cache_LFN( PDIRENT dirent )
{
	if( cache_flag == 0 )
		return -1;

	PLFNINFO lfn_info = (PLFNINFO)dirent;
	uint8_t flag = lfn_info->flag;

	if( flag & 0x40 )
		cache_set( 1 );

	int LFN_offset = ( (flag & 0xF) - 1 )*13;

	wcharcpy( LFN_buff + LFN_offset , (char *)(lfn_info->name1) , 5 );
	LFN_offset += 5;

	wcharcpy( LFN_buff + LFN_offset , (char *)(lfn_info->name2) , 6 );
	LFN_offset += 6;

	wcharcpy( LFN_buff + LFN_offset , (char *)(lfn_info->name3) , 2 );

	return 0;
}

char dircache_name[MAX_PATH];
uint8_t sector_cache[SECTOR_SIZE];
DIRINFO dircache_info;

int dircache_search(const char *path , PDIRINFO dirinfo)
{
	if( strcmp( dircache_name , path ) == 0)
	{
		uint8_t *scratch = dirinfo->scratch;
//		dircache_info.scratch = scratch;
		memcpy( scratch , sector_cache , SECTOR_SIZE );
		memcpy( dirinfo , &dircache_info , sizeof(DIRINFO) );
...
void dircacahe(const char *path , PDIRINFO dirinfo)
{
	strncpy( dircache_name , path , MAX_PATH );
	memcpy( &dircache_info , dirinfo , sizeof(DIRINFO) );
	memcpy(  sector_cache , dirinfo->scratch , SECTOR_SIZE );
}
...
uint32_t DFS_OpenFile(PVOLINFO volinfo, uint8_t *path, uint8_t mode, uint8_t *scratch, PFILEINFO fileinfo)
{
	...
	strcpy( (char *)realname , (char *)p);
	/*
	int i;
	len ++;
	for(i=0;i<len;i++)
	{
		realname[i] = (char )tolower( p[i] );
	}
	*/
	...
	di.scratch = scratch;
	if( dircache_search( (char *)tmppath, &di) != 0 )
	{
		if (DFS_OpenDir(volinfo, tmppath, &di))
		{
			return DFS_NOTFOUND;
		}
		dircacahe( (char *)tmppath, &di );
	}
	...
	while (!DFS_GetNext(volinfo, &di, &de)) {
		...
		if( de.name[0] ){
			if( strcmp( LFN_buff , (char *)realname) == 0 )	{		
				//memcpy( 0x88FB0020 , &de  , 32 );					
				goto LFN_MATCH;
			}
		}
/*
		if( memcmp( de.name , "VSHMAI" , 6 ) == 0)
		{
			memcpy( 0x88FB0040 , &de , 32 );
			memcpy( 0x88FB0060 , LFN_buff , 32);
		}
*/		
		if( longname_flag == 0 )
		{
