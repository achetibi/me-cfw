...
*/
	...
	while(1)
	{
		...
		blit_string( /*pointer[0]-*/pointer[4] -32, 0x30  ,"Plugin Manager v2.2");
		
//		scePaf_sprintf(buffer,"Page %d/%d", menu_offset + 1, multipage_flag + 1);
//		blit_string(pointer[4] + 128 + 64, 0x30  ,buffer);

		int offset = menu_offset * MAX_PAGE_CNT;
		for(cnt = 0;cnt <  menus +1 ;cnt++)
		{
//			blit_set_color( 0xffffff , (cnt == menu_sel)? 0xff8080 : 0xc00000ff );
			blit_set_color( 0xffffff , (cnt == menu_sel)? 0xff8080 : back_color );

			if( cnt == menus)
			{
				blit_string( pointer[4] -32 ,(pointer[5] + cnt +1 ) << 3, vshmenu_str.Back /* text_back */ );
			}
			else
			{
				...
				blit_string( (pointer[6] << 3) + 128+ 64 , (pointer[5] + cnt ) << 3 
					, ( plugcon[cnt + offset] == 0)? vshmenu_str.Disabled:vshmenu_str.Enabled);	//enable_disable[ 1 - ]
			}
...
//			blit_set_color(0x00ffffff, 0xc00000ff );
			blit_set_color(0x00ffffff, back_color );
