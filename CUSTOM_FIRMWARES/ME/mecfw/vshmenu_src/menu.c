...
/*		char *device;
		...
		*/
		...
		bridge = (cnf.usbdevice == 5 )?"UMD Disc":"Memory Stick";
	}	
	item_str[TMENU_USB_DEVICE] = bridge;

	//umd mode
	{
		cnf.umdmode = limit( cnf.umdmode , 0 , 4);
	}
...
int menu_ctrl(u32 buttons,u32 button_on)
{
	...
	if(direction>-2)
	{
		...
		case TMENU_XMB_PLUGINS:
			//if(direction) 			
			change_plugins(direction , 0 );
			break;
		case TMENU_GAME_PLUGINS:
			//if(direction) 			
			change_plugins(direction , 1 );
			break;
		case TMENU_POPS_PLUGINS:
			//if(direction) 			
			change_plugins(direction , 2 );
			break;
		case TMENU_PLUGINS_MGR:
