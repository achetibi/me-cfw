...
int module_start(SceSize args, void * argp)
{
	...
	// if(api == 0x120 || api == 0x123 || api == 0x125) {
		...
		SceModule * utility = (SceModule *) sceKernelFindModuleByName("sceUtility_Driver");
		...
		if(utility != NULL) {
			...
			if(result == 0) {
				...
				if(result == 0) {
					// Disable Home Menu
					sctrlHENPatchSyscall(sctrlHENFindFunction("sceLoadExec", "LoadExecForUser", 0x4AC57943), cbdeny);
